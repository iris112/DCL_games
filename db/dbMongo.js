...
userPlayerInfos.set('toJSON', {
	transform: (doc, ret) => {
		if (ret.totalBetAmount) ret.totalBetAmount = Number(ret.totalBetAmount.toString());
		if (ret.totalAmountWin) ret.totalAmountWin = Number(ret.totalAmountWin.toString());
		return ret;
	}
});
